...
* **文書名**：EDINET 決算短信ビューア アプリ ソフトウェア要求仕様書（SRS）
* **作成者**：
* **バージョン**：v0.1（ドラフト）
* **作成日**：2025-xx-xx
* **更新履歴**：
...
本書は、EDINET から企業の決算短信を取得し、その内容を Streamlit アプリとして表示するツール（以下「本アプリ」）のソフトウェア要求を定義することを目的とする。また、本アプリは以下の目的も併せて持つ：

* 決算短信を **GitHub Pages で作成するブログ用に Markdown に変換** すること
...
本アプリは、EDINET から決算短信（四半期・通期）を取得し、以下を実現する：
...
本アプリは、個人・社内向けの決算閲覧用ツールであり、Python/Streamlit を用いてローカル環境で動作する Web UI アプリとして提供される。
